...
public class Aarroute implements IOFMessageListener, IFloodlightModule {
	...
	public net.floodlightcontroller.core.IListener.Command receive(
		...
		int packet_length = ipPacket.getTotalLength(); // msg.getLengthU();
		...
		if (found) {
			...
			if (pktcount_map.get(temp_pkt_count).size() <= 10) {
				...
				/*
				 * System.out.println("Computing class"); Map<Long,
				 * packet_interval> class_finder = pktcount_map
				 * .get(temp_pkt_count); long avg_inter_pkt_delay = 0,
				 * avg_pkt_length = 0; for (Map.Entry<Long, packet_interval>
				 * class_it : class_finder .entrySet()) { avg_inter_pkt_delay +=
				 * class_it.getValue().inter_pkt_delay; avg_pkt_length +=
				 * class_it.getValue().packet_size; }
				 * 
	...
	private boolean routeFlow(IOFSwitch inSwitch, OFPacketIn pi,
		...
		/*
		 * if (match.getTransportDestination() < 5000) { // Class 1
		 * System.out.println("Sensitive Traffic--------< 5000 Class 1"); } else
		 ...
		 * path_iter.nextIndex()); if (path_iter.hasNext()) path_iter.next();
		 * System.out.println("Iterating" + path_iter.nextIndex());
		 * System.out.println("Iterating" + path_iter.nextIndex()); if
		 * (path_iter.hasNext()) path_iter.next();
		 * System.out.println("Iterating" + path_iter.nextIndex());
		 * System.out.println("Sensitive Traffic--------> 7000 <8000 Class 4");
		 * }
		 * 
		 * else { System.out.println("Class 5");
		 * System.out.println("In Sensitive Traffic"); while
		 * (path_iter.hasNext()) { System.out.println("Iterating" +
		 * path_iter.nextIndex()); path_iter.next();
		 * System.out.println("Iterating" + path_iter.nextIndex());
		 * System.out.println("======================================"); }
		 * path_iter.previous(); }
		 */
		//int pathSelected = 0;
		net.floodlightcontroller.kshortestpath.Path pathSelected;
		if (pkt != null) {
			...
			int classLabel=0;
			  ...
			  System.out.println("Sensitive Traffic--------< 5000 Class 1"); 
			  classLabel =1;
			  } else if (match.getTransportDestination() > 5000 && match.getTransportDestination() < 6000) { 
				  classLabel=2;
			  } else if (match.getTransportDestination() > 6000 &&
			...
			System.out.println("Entered ML part");

			Map<Long, packet_interval> class_finder = pktcount_map.get(pkt);
			...
			long requiredBandwidthApplication = avg_inter_pkt_delay
					* avg_pkt_length;

			while (path_iter.hasNext()) {
				net.floodlightcontroller.kshortestpath.Path path = path_iter
						.next();
				List<net.floodlightcontroller.kshortestpath.BaseVertex> vtx = path
						.get_vertices();
				Iterator<net.floodlightcontroller.kshortestpath.BaseVertex> vtx_iter = vtx
				...
				short inport = 0, outport;
				int flag = 1;
				net.floodlightcontroller.kshortestpath.BaseVertex last_switch = null;
				
				net.floodlightcontroller.kshortestpath.BaseVertex sw1 = null;
				boolean feasible=true;
				while (vtx_iter.hasNext()) {
					if (flag == 1) {
						last_switch = vtx_iter.next();
						sw1 = vtx_iter.next();
						flag = 0;
					...
					if(requiredBandwidthApplication < graph.get_edge_weight(last_switch, sw1)){
						feasible=false;
						break;
					}
					last_switch=sw1;
			...
			if(feasible_paths.size()>0){
				//TODO modify ML 
				pathSelected = feasible_paths.get(0);
			}else{
				//TODO modify ML
				pathSelected = p.get(0);
			}

		} else {

			pathSelected = p.get((int) (Math.random() * p.size()));
			
		}

		//net.floodlightcontroller.kshortestpath.Path path = path_iter.next();
		List<net.floodlightcontroller.kshortestpath.BaseVertex> vtx = pathSelected
				.get_vertices();
		Iterator<net.floodlightcontroller.kshortestpath.BaseVertex> vtx_iter = vtx
				.iterator();
